...
from dataset import TrainDataDynamicLocal,ValidData
from models import UNet,DynamicContextDiscriminator

logging.basicConfig(filename="log_cardiac_local_dyn.txt",
                            filemode='a',
...
def create_datasets(args):
    ...
    train_data = TrainDataDynamicLocal(args.train_path)
...
def train_epoch(args, epoch, modelG, modelD, data_loader, optimizerG, optimizerD, writer, display_loader, exp_dir):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        input, input_canvas, target, target_canvas, coord, bbox = data

        input = input.squeeze(0).to(args.device)
        input_canvas = input_canvas.squeeze(0).to(args.device)
        target = target.squeeze(0).to(args.device)
        target_canvas = target_canvas.squeeze(0).to(args.device)

        input = input.float()
        input_canvas = input_canvas.float()
        target = target.float()
        target_canvas = target_canvas.float()

        batch_size = input.shape[0]

        outG = modelG(input)
        outG_canvas = F.pad(outG,pad=(75,75,75,75),mode='constant')
        ...
        for i in range(input.shape[0]):
            ...
            cy = int(cy.item())
            cx,cy = cx+75,cy+75
            o.append(torch.exp(outG_canvas[i,:,cy-size:cy+size,cx-size:cx+size]))
            t.append((make_one_hot(target_canvas[i,:,cy-size:cy+size,cx-size:cx+size].unsqueeze(0).long()).squeeze(0)))
...
def build_model(args):
    model = UNet(1,4).to(args.device)
    return model

def build_discriminator():

    netD = DynamicContextDiscriminator(n_channels=4).to(args.device)
