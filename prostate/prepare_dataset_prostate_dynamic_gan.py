...
h5_dir = '/media/htic/NewVolume3/Balamurali/promise_prostate_dataset/train/'
...
h5_save_dir = '/media/htic/NewVolume3/Balamurali/promise_prostate_dataset/train_dyn_16_mod/'
h5_count = 1

for kk in tqdm(range(20,90,10)):
    files = files_np[np.logical_and(dim_np >= kk,dim_np < kk + 10)]
    len_files = len(files)
    #if not len_files:
    #    continue
    
    # Files convert to numpy stack
    img_np  = np.empty([128,128,0])
    mask_np = np.empty([128,128,0])
    ...
    img_np_aug = np.empty([128,128,0])
    mask_np_aug = np.empty([128,128,0])
    ...
    for jj in  range(len_rem):
        ...
        tform_img  = tform_img_mask[:,:128]
        tform_mask = tform_img_mask[:,128:]
    ...
    print ("{}:{}---{}-->{}".format(kk,kk+10,len_files,batch_split))
    ...
    for jj in range(batch_split):
        ...
        for ii in range(batch_size):
            rp = measure.regionprops(mask[:,:,ii])[0]
